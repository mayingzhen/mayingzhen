...
ILAPI ILenum ILAPIENTRY ilTypeFromExt(const ILstring FileName)
{
	...
	if(FileName == NULL || strlen(FileName) < 1)
	{
		...
		if(!iStrCmp(Ext, IL_TEXT("pvr")))
			return IL_PVR;
		if(!iStrCmp(Ext, IL_TEXT("tga")) || !iStrCmp(Ext, IL_TEXT("vda")) || !iStrCmp(Ext, IL_TEXT("icb")) || !iStrCmp(Ext, IL_TEXT("vst")))
			return IL_TGA;
		...
		if(!iStrCmp(Ext, IL_TEXT("png")))
			return IL_PNG;
		if(!iStrCmp(Ext, IL_TEXT("atc")))
			return IL_ATC;
		if(!iStrCmp(Ext, IL_TEXT("ktx")))
			return IL_KTX;
		if(!iStrCmp(Ext, IL_TEXT("mjp")))
			return IL_MJP;
		...
		if(!iStrCmp(Ext, IL_TEXT("pix")))
			return IL_PIX;
		if(!iStrCmp(Ext, IL_TEXT("pbm")) || !iStrCmp(Ext, IL_TEXT("pgm")) || !iStrCmp(Ext, IL_TEXT("pnm")) || !iStrCmp(Ext, IL_TEXT("ppm")))
			return IL_PNM;
		...
		if(!iStrCmp(Ext, IL_TEXT("pxr")))
			return IL_PXR;
		if(!iStrCmp(Ext, IL_TEXT("sgi")) || !iStrCmp(Ext, IL_TEXT("bw")) || !iStrCmp(Ext, IL_TEXT("rgb")) || !iStrCmp(Ext, IL_TEXT("rgba")))
			return IL_SGI;
