...

{
    ...
    // fourcc formats
        ...
        // if (Format == PF_G16R16F || Format == PF_R32F || dxt)
    ...
    for(i = 0; i < CUBEMAP_SIDES; i++)
    {
        ...
        if(Head.ddsCaps2 & CubemapDirections[i])
        {
            if(i != 0)
            {
                ...
                if(CompFormat == PF_R16F || CompFormat == PF_G16R16F || CompFormat == PF_A16B16G16R16F || CompFormat == PF_R32F || CompFormat == PF_G32R32F ||
...
ILboolean iLoadDdsInternal()
{
    ...
    if(!(Head.Flags1 & (DDS_LINEARSIZE | DDS_PITCH)) || Head.LinearSize == 0)
    {
...
ILvoid AdjustVolumeTexture(DDSHEAD *Head)
{
    ...
    case PF_LUMINANCE_ALPHA:
        ...
        Head->LinearSize = IL_MAX(1, Head->Width) * IL_MAX(1, Head->Height) * (Head->RGBBitCount / 8);
        break;
    ...
    case PF_A32B32G32R32F:
        Head->LinearSize = IL_MAX(1, Head->Width) * IL_MAX(1, Head->Height) * iCompFormatToBpp(CompFormat);
        break;
    }
...
ILboolean AllocImage()
{
    ...
    case PF_LUMINANCE:
		if(Head.RGBBitCount == 16 && Head.RBitMask == 0xFFFF)
        {                   // HACK
...
/* Assumes that the global variable CompFormat stores the format of the global
   pointer CompData (that's the pointer to the compressed data). Decompresses this
   data into Image->Data, returns if it was successful. It also uses the globals
   Width and Height. Assumes that iCurImage has valid Width, Height, Depth, Data,
   SizeOfData, Bpp, Bpc, Bps, SizeOfPlane, Format and Type fields. It is more or
   less assumed that the image has u8 rgba data (perhaps not for float images...)
   TODO: don't use globals, clean this function (and this file) up  */
ILboolean Decompress()
{	
	if(ilGetInteger(IL_DXT_DONT_DECOMP) == IL_TRUE)
	{
		return IL_TRUE;
	}
...
ILAPI ILboolean ILAPIENTRY ilDxtcDataToSurface()
{
    ...
    if(!(iCurImage->DxtcFormat == IL_DXT1 || iCurImage->DxtcFormat == IL_DXT3 || iCurImage->DxtcFormat == IL_DXT5))
    {
