 ...
 */
ILfloat Maximize(Box* cube, ILubyte dir, ILint first, ILint last, ILint* cut, ILint whole_r, ILint whole_g, ILint whole_b, ILint whole_w)
{
	...
	*cut = -1;
	...
	for(i = first; i < last; ++i)
	{
		...
		{
			temp = ((ILfloat) half_r * half_r + (ILfloat) half_g * half_g + (ILfloat) half_b * half_b) / half_w;
		}
...
ILimage* iQuantizeImage(ILimage* Image, ILuint NumCols)
{
	...
	mask_1.load = (vector unsigned int)
	{
		0xFF0000FF, 0x0000FF00, 0x00FF0000, 0xFF0000FF
	};
	mask_2.load = (vector unsigned int)
	{
