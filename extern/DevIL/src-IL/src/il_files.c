...
{
	ilSetRead(iDefaultOpenR, iDefaultCloseR, iDefaultEof, iDefaultGetc, iDefaultRead, iDefaultRSeek, iDefaultRTell);
	return;
}

ILvoid ILAPIENTRY ilResetWrite()
{
	ilSetWrite(iDefaultOpenW, iDefaultCloseW, iDefaultPutc, iDefaultWSeek, iDefaultWTell, iDefaultWrite);
	return;
}
...
ILuint ILAPIENTRY iReadLump(ILvoid* Buffer, ILuint Size, ILuint Number)
{
	...
	for(i = 0; i < ByteSize; i++)
	{
		*((ILubyte*)Buffer + i) = *((ILubyte*)ReadLump + ReadLumpPos + i);
		if(ReadLumpSize > 0)
		{							// ReadLumpSize is too large to care about apparently
...
ILint ILAPIENTRY iWriteLump(const ILvoid* Buffer, ILuint Size, ILuint Number)
{
	...
	for(; i < SizeBytes; i++)
	{
		if(WriteLumpSize > 0)
		{
			if(WriteLumpPos + i >= WriteLumpSize)
			{						// Should we use > instead?
				ilSetError(IL_FILE_WRITE_ERROR);
				WriteLumpPos += i;
				return i;
