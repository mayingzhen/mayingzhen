...
ILuint code_mask[13] = { 0L, 0x0001L, 0x0003L, 0x0007L, 0x000FL, 0x001FL, 0x003FL, 0x007FL, 0x00FFL, 0x01FFL, 0x03FFL, 0x07FFL, 0x0FFFL };
...
	while((c = get_next_code()) != ending && Read < Height)
	{
		if(c == clear)
		{
			...
			if(c >= slot)
				c = 0;
			oc = fc = c;
			if(DisposalMethod == 1 && !Gfx->Used && Gfx->Transparent == c && (Gfx->Packed & 0x1) != 0) i++;
		...
		else
		{
			...
			*sp++ = (ILbyte) code;
			...
			{
				sp--;
				if(DisposalMethod == 1 && !Gfx->Used && Gfx->Transparent == *sp && (Gfx->Packed & 0x1) != 0) i++;
