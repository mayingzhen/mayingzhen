...
ILboolean ilIsValidPnm(const ILstring FileName)
{
	...
	if(!iCheckExtension(FileName, IL_TEXT("pbm")) && !iCheckExtension(FileName, IL_TEXT("pgm")) && !iCheckExtension(FileName, IL_TEXT("ppm")) && !iCheckExtension(FileName, IL_TEXT("pnm")))
	{
...
ILboolean iLoadPnmInternal()
{
	...
	if(Info.Type == IL_PBM_ASCII || Info.Type == IL_PBM_BINARY || Info.Type == IL_PGM_ASCII || Info.Type == IL_PGM_BINARY)
	{
...
ILimage* ilReadAsciiPpm(PPMINFO* Info)
{
	...
	{			// && !feof(File)) {
		...
		while((LineBuffer[LineInc] != NUL) && (LineBuffer[LineInc] != '\n'))
		{
			...
			while(!isalnum(LineBuffer[LineInc]) && LineBuffer[LineInc] != NUL)
			{			// Skip any whitespace
				LineInc++;
			}
...
ILboolean iGetWord(ILvoid)
{
	...
	while(Looping)
	{
		...
		if(Current == '#')
		{				// '#' is a comment...read until end of line
			while((Current = igetc()) != IL_EOF && Current != '\n');
		}
...
ILboolean iSavePnmInternal()
{
	...
	if(MaxVal > UCHAR_MAX && Type >= IL_PBM_BINARY)
	{					// binary cannot be higher than 255
		ilSetError(IL_FORMAT_NOT_SUPPORTED);
		return IL_FALSE;
	}
