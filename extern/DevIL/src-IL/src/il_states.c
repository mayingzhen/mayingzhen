...
ILstring _ilVersion = IL_TEXT("Developer's Image Library (DevIL) 1.6.8pre ");

ILstring _ilLoadExt = ""IL_BMP_EXT IL_CUT_EXT IL_DCX_EXT IL_DDS_EXT IL_GIF_EXT IL_HDR_EXT IL_ICO_EXT IL_JPG_EXT IL_LIF_EXT IL_MDL_EXT IL_MNG_EXT IL_PCX_EXT IL_PIC_EXT IL_PIX_EXT IL_PNG_EXT IL_PNM_EXT IL_PSD_EXT IL_PSP_EXT IL_PXR_EXT IL_SGI_EXT IL_TGA_EXT
	IL_TIF_EXT IL_WAL_EXT IL_XPM_EXT;
ILstring _ilSaveExt = ""IL_BMP_EXT IL_CHEAD_EXT IL_DDS_EXT IL_JPG_EXT IL_PCX_EXT IL_PNG_EXT IL_PNM_EXT IL_PSD_EXT IL_SGI_EXT IL_TGA_EXT IL_TIF_EXT;
...
ILvoid ilDefaultStates()
{
	...
	ilStates[ilCurrentPos].ilDontDecompDxtcData = IL_FALSE;
...
ILvoid ILAPIENTRY ilGetIntegerv(ILenum Mode, ILint* Param)
{
	...
	*Param = 0;
	...
	case IL_DXT_DONT_DECOMP:
		*Param = ilStates[ilCurrentPos].ilDontDecompDxtcData;
		break;
...
ILvoid ILAPIENTRY iGetIntegervImage(ILimage* Image, ILenum Mode, ILint* Param)
{
	...
	*Param = 0;
	...
	case IL_IMAGE_FORMAT:
		if(ilGetInteger(IL_KEEP_DXTC_DATA) == IL_TRUE && (Image->DxtcData != NULL) && (Image->DxtcSize > 0))
		{
			*Param = Image->DxtcFormat;
		}
		else
	...
	case IL_PALETTE_BASE_TYPE:
		...
		case IL_IMAGE_PVR_ADDBORDER:
		{
			*Param = Image->PvrAddBorder;
			break;
		}
	default:
		ilSetError(IL_INVALID_ENUM);
...
ILvoid ILAPIENTRY ilSetInteger(ILenum Mode, ILint Param)
{
	...
	case IL_DXT_DONT_DECOMP:
		if(Param == IL_FALSE || Param == IL_TRUE)
		{
			ilStates[ilCurrentPos].ilDontDecompDxtcData = Param;
			return;
		}
		break;		
