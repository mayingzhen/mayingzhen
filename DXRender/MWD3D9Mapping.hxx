...
const char* D3D9Mapping::GetHLSLProfile(SHADER_PROFILE Profile)
{
    switch (Profile)
    {
    case SP_VS_2_0:
...
const char* D3D9Mapping::GetD3DErrorDescription(HRESULT hr)
{
    return "";//DXErrorDescription(hr);
}

const char* D3D9Mapping::GetD3DErrorString(HRESULT hr)
{
...
DWORD D3D9Mapping::GetD3DFilterType(TEXTURE_FILTER_TYPE Type)
{
    if (Type == TEXF_DEFAULT)
        Type = TEXF_LINEAR;//RenderHelper::Instance()->GetDefaultTextureFilter();
...
const char* D3D9Mapping::GetMSAAString(MULTI_SAMPLE msaa)
{
    switch(msaa)
    {
    case MSAA_NONE:
...
const char* D3D9Mapping::GetUsageString(USAGE Usage)
{
    const char* sUsage = "";
...
const char* D3D9Mapping::GetFormatString(FORMAT Format)
{
    switch (Format)
    {
    case FMT_UNKNOWN:
...
const char* D3D9Mapping::GetShaderProfileString(SHADER_PROFILE profile)
{
    switch (profile)
    {
    case SP_VS_2_0:
