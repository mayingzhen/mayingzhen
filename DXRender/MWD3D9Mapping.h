...
#define D3DErrorExceptionFunction(func, hr) assert(hr == D3D_OK)

namespace ma
{

struct DXRENDER_API D3D9RenderState
{
    DWORD       CullMode;
    DWORD       FillMode;
    DWORD       ShadeMode;
...
struct DXRENDER_API D3D9DisplayDevice
{
    UINT                    AdapterNumber;
...
public:
    ...
    static const char*   GetHLSLProfile(SHADER_PROFILE Profile);

    static int          GetTypeSize(SHADER_PARAM_TYPE Type);

    static D3DXIMAGE_FILEFORMAT GetD3DXImageFileFormat(IMAGE_FILE_FORMAT Format);

    static const char*   GetD3DErrorDescription(HRESULT hr);
    static const char*   GetD3DErrorString(HRESULT hr);
    ...
    static const char*   GetMSAAString(MULTI_SAMPLE msaa);
    static const char*   GetUsageString(USAGE usage);
    static const char*   GetFormatString(FORMAT format);
    static const char*   GetShaderProfileString(SHADER_PROFILE profile);
};
