...
//{
//	assert (Math::IsPowerOfTwo(mWidth) && Math::IsPowerOfTwo(mHeight));
//	assert(mWidth == mHeight)
//	assert (mUsage == USAGE_STATIC);
//
//	int level0 = 0;
//	int leveln = 1;
//	int size0 = mWidth;
...
void D3D9Texture::ResetDevice()
{
    if (mUsage == USAGE_DYNAMIC)
    {
        ...
        if (rWidth == -1 || rHeight == -1)
        {
			...
			D3DPRESENT_PARAMETERS* dp = pRenderDevice->GetD3dPP();
			rWidth = dp->BackBufferWidth;
			rHeight = dp->BackBufferHeight;
        }
...
// {
//     SAFE_RELEASE(mD3D9VolumeTexture);
// }
// 
