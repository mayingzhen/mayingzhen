#include "PVRTCDecompress.h"
namespace ma
{

// ---------------------------------------------------------------------
// Referenced from:http://code.google.com/p/oolongengine/source/browse/trunk/Oolong%20Engine2/Renderer/Core/GraphicsDevice/Texture.mm?r=46
// ---------------------------------------------------------------------
...
static uint32 TwiddleUV(uint32 YSize, uint32 XSize, uint32 YPos, uint32 XPos)
{
	...
	ASSERT(POWER_OF_2(YSize) != 0);
	ASSERT(POWER_OF_2(XSize) != 0);
...
int ETC1Decompress(const void * const pSrcData,
...
void ETC1Decompress( IN OUT ImageData& imgData )
{
	...
	for(size_t mip = 0; mip<= imgData.num_mipmaps; ++mip)
	{
		for(size_t i = 0; i < faces; ++i)
		{
			...
			ETC1Decompress(src.data, nwidth, nheight, output->GetPtr() + nOffset, 0);
...
static void DecompressDXT( unsigned char* rgba, const void* block, PixelFormat format)
{
	// get the block locations
	void const* colourBlock = block;
	void const* alphaBock = block;
	if( format == PF_DXT3 || format == PF_DXT5)
		colourBlock = reinterpret_cast< unsigned char const* >( block ) + 8;

	// decompress colour
	DecompressColourDXT( rgba, colourBlock, format == PF_DXT1 );
...
void DecompressImageDXT( unsigned char* rgba, const void* blocks, int width, int height, int depth, PixelFormat format )
{
	// initialise the block input
	unsigned char const* sourceBlock = reinterpret_cast< unsigned char const* >( blocks );
	int bytesPerBlock = format == PF_DXT1 ? 8 : 16;
