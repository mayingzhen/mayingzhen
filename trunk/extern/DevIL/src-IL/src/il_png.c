...
ILboolean iSavePngInternal()
{
	...
	*/
	...
	{
		png_set_IHDR(png_ptr, info_ptr, iCurImage->Width, iCurImage->Height, BitDepth, PngType, PNG_INTERLACE_ADAM7, PNG_COMPRESSION_TYPE_BASE, PNG_FILTER_TYPE_BASE);
	}
	else
	{
		png_set_IHDR(png_ptr, info_ptr, iCurImage->Width, iCurImage->Height, BitDepth, PngType, PNG_INTERLACE_NONE, PNG_COMPRESSION_TYPE_BASE, PNG_FILTER_TYPE_BASE);
	}
	...
	if(iCurImage->Format == IL_COLOUR_INDEX)
	{
		...
		png_set_PLTE(png_ptr, info_ptr, (png_colorp) TempPal->Palette, ilGetInteger(IL_PALETTE_NUM_COLS));
