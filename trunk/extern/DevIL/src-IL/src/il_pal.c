...
char* iFgetw(ILubyte* Buff, ILint MaxLen, FILE* File)
{
	...
	for(i = 0; i < MaxLen - 1; i++)
	{
		...
		{
			while(Temp == ' ')
			{	// Just to get rid of any extra spaces
				Temp = fgetc(File);
			}
		...
		if(!isprint(Temp))
		{			// Skips any non-printing characters
			while(!isprint(Temp))
			{
				Temp = fgetc(File);
			}
	...
	ILboolean ilSaveJascPal(const ILstring FileName)
	{
		...
		ILboolean ilLoadColPal(const ILstring FileName)
		{
			...
			{		// has a header
				...
				if((FileSize - 8) % 3 != 0)
				{	// check to make sure an even multiple of 3!
					icloser(ColFile);
					ilSetError(IL_ILLEGAL_FILE_VALUE);
					return IL_FALSE;
				}
		...
		ILAPI ILpal * ILAPIENTRY iCopyPal ()
		{
			...
			if(iCurImage == NULL || iCurImage->Pal.Palette == NULL || iCurImage->Pal.PalSize == 0 || iCurImage->Pal.PalType == IL_PAL_NONE)
			{
				ilSetError(IL_ILLEGAL_OPERATION);
				return NULL;
			}
		...
		return NULL;
			...
			case IL_PAL_BGRA32:
				...
				case IL_PAL_BGR24:
					...
					if(NewPal->Palette == NULL)
						goto alloc_error;
					if((Pal->PalType == IL_PAL_BGR24 && (DestFormat == IL_PAL_RGB32 || DestFormat == IL_PAL_RGBA32)) || (Pal->PalType == IL_PAL_RGB24 && (DestFormat == IL_PAL_BGR32 || DestFormat == IL_PAL_BGRA32)))
					{
		...
		ILboolean ILAPIENTRY ilApplyPal(const ILstring FileName)
		{
			...
			case IL_LUMINANCE_ALPHA:
				for(i = 0; i < CurImage->SizeOfData; i += CurImage->Bpp)
				{
					...
					{
						// No need to perform a sqrt.
						PalInfo[j] = ((ILuint) CurImage->Data[i] - (ILuint) Image.Pal.Palette[j * 3]) * ((ILuint) CurImage->Data[i] - (ILuint) Image.Pal.Palette[j * 3]) + ((ILuint) CurImage->Data[i] - (ILuint) Image.Pal.Palette[j * 3 + 1]) * ((ILuint) CurImage->Data[i] - (ILuint) Image.Pal.Palette[j * 3 + 1]) + ((ILuint) CurImage->Data[i] - (ILuint) Image.Pal.Palette[j * 3 + 2]) * ((ILuint) CurImage->Data[i] - (ILuint) Image.Pal.Palette[j * 3 + 2]);
					}
			...
			if(!ilTexImage(iCurImage->Width, iCurImage->Height, iCurImage->Depth, 1, IL_COLOUR_INDEX, IL_UNSIGNED_BYTE, NewData))
			{
