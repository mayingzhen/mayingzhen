...
mng_bool MNG_DECL mymngwritestream(mng_handle mng, mng_ptr pBuf, mng_uint32 iSize, mng_uint32* iWritten)
{
	*iWritten = iwrite(pBuf, 1, iSize);
	return MNG_TRUE;
}
...
// Error Callback;
//---------------------------------------------------------------------------------------------
mng_bool MNG_DECL mymngerror(mng_handle mng, mng_int32 code, mng_int8 severity, mng_chunkid chunktype, mng_uint32 chunkseq, mng_int32 extra1, mng_int32 extra2, mng_pchar text)
{
	// error occured;
	return MNG_FALSE;
}
...
ILboolean ILAPIENTRY ilMakeMngAnimFile(const ILstring FileName, ILint nWidth, ILint nHeight, ILint nFrames, ILvoid ** pFrameImageDatas, ILint nTick)
{
	mng_handle hmng;
	mng_retcode res;
	ILint i, y;
	uLongf dwRawLen;
	unsigned int dwLineImageBytes, dwLineDataBytes, dstBufferSize;
	unsigned char*	tmpBuffer, *dstBuffer;
	unsigned char*	pSourceImageData;

	ILHANDLE File = iopenw(FileName);
	if(File == NULL)
	{
		ilSetError(IL_COULD_NOT_OPEN_FILE);
		return IL_FALSE;
	...
	// Initialize mng env!
	hmng = mng_initialize(NULL, mymngalloc, mymngfree, MNG_NULL);
	...
	// Create mng file in momery
	res = mng_create(hmng);

	// Set mng HEADER, and define a image object, then define a PNG data stream.
	mng_putchunk_mhdr(hmng, nWidth, nHeight, nTick, 1, nFrames, nFrames / nTick + 1, MNG_SIMPLICITY_TRANSPARENCY);

	dwLineImageBytes = nWidth * 4;
	dwLineDataBytes = dwLineImageBytes + 1;
	dstBufferSize = dwLineDataBytes * nHeight;
	tmpBuffer = icalloc(1, dstBufferSize);	// for format translation
	dstBuffer = icalloc(1, dstBufferSize);	// for zip

	// png's origin point is at UpperLeft
	for(i = 0; i < nFrames; i++)
	{
		pSourceImageData = pFrameImageDatas[i];
		mng_putchunk_defi(hmng, i + 1, 0, 0, MNG_FALSE, 0, 0, MNG_FALSE, 0, 0, 0, 0);
		mng_putchunk_ihdr(hmng, nWidth, nHeight, MNG_BITDEPTH_8, MNG_COLORTYPE_RGBA, MNG_COMPRESSION_DEFLATE, MNG_FILTER_ADAPTIVE, MNG_INTERLACE_NONE);

		// Copy original data into stupid png format
		for(y = 0; y < nHeight; y++)
		{
			// First Byte in each Scanline is Filterbyte: Currently 0 -> No Filter.
			tmpBuffer[y * dwLineDataBytes] = 0;

			// Copy the scanline in reverse order
			memcpy(tmpBuffer + y * dwLineDataBytes + 1, pSourceImageData + y * dwLineImageBytes, dwLineImageBytes);
		}

		// Compress data with ZLib (Deflate):
		dwRawLen = dstBufferSize;
		compress2((Bytef*)dstBuffer, &dwRawLen, (const Bytef*)tmpBuffer, dstBufferSize, 9);

		// Write end chunk into MNG File:
		mng_putchunk_idat(hmng, dwRawLen, (mng_ptr*)dstBuffer);
	...
	mng_putchunk_save(hmng, MNG_TRUE, MNG_SAVEOFFSET_4BYTE, 0);
	mng_putchunk_term(hmng, MNG_TERMACTION_REPEAT, MNG_ITERACTION_LASTFRAME, 0, 0X7FFFFFFF);
	mng_putchunk_seek(hmng, 8, "linghuye");
	mng_putchunk_show(hmng, MNG_FALSE, 1, nFrames, MNG_SHOWMODE_0);

	mng_putchunk_mend(hmng);

	// Write into file and then close it!
	res = mng_write(hmng);
	iclosew(File);
	mng_cleanup(&hmng);
	return IL_TRUE;
