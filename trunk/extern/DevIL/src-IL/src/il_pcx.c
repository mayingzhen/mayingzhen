...
ILboolean iUncompressPcx(PCXHEAD* Header)
{
	...
	for(y = 0; y < iCurImage->Height; y++)
	{
		...
		for(x = 0; x < iCurImage->Width; x++)
		{
			...
			{
				iCurImage->Data[y * iCurImage->Bps + x * iCurImage->Bpp + c] = ScanLine[x + c * Header->Bps];
			}
		}
	}
	...
	{
		x = itell();
		if(iread(&ByteHead, 1, 1) == 0)
		{						// If true, assume that we have a luminance image.
...
ILboolean iUncompressSmall(PCXHEAD* Header)
{
	...
	{	// 4-bit images
		...
		for(y = 0; y < iCurImage->Height; y++)
		{
			...
			for(x = 0; x < iCurImage->Width; x++)
			{				// 'Cleverly' ignores the pad bytes. ;)
