...
ILvoid ILAPIENTRY ilGenImages(ILsizei Num, ILuint* Images)
{
	...
	{
		if(FreeNames != NULL)
		{						// If any have been deleted, then reuse their image names.
...
ILboolean ILAPIENTRY ilHasMipMap()
{
	return (iCurImage->Mipmaps != NULL);
}
...
ILvoid ILAPIENTRY ilShutDown()
{
	...
	if(!IsInit)
	{	// Prevent from being called when not initialized.
		ilSetError(IL_ILLEGAL_OPERATION);
		return;
	}
