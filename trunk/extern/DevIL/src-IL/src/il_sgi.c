...
ILboolean iLoadSgiInternal()
{
	...
	{	// Non-RLE  //(Header.Storage == SGI_VERBATIM)
...
ILboolean iReadRleSgi(iSgiHeader* Head)
{
	...
	for(ixPlane = 0; ixPlane < Head->ZSize; ixPlane++)
	{
		for(ixHeight = 0, Cur = 0; ixHeight < Head->YSize; ixHeight++, Cur += Head->XSize * Head->Bpc)
		{
			...
			if(iGetScanLine((TempData[ixPlane]) + (ixHeight * Head->XSize * Head->Bpc), Head, RleLen) != Head->XSize * Head->Bpc)
			{
	...
	for(ixPixel = 0; ixPixel < iCurImage->SizeOfData; ixPixel += Head->ZSize * Head->Bpc, ChanInt += Head->Bpc)
	{
...
ILint iGetScanLine(ILubyte* ScanLine, iSgiHeader* Head, ILuint Length)
{
	...
	while(BppRead < Length && CurPos < Bps)
	{
		...
		if(!(Count = (Pixel & 0x7f)))	// If 0, line ends
			return CurPos;
		if(Pixel & 0x80)
		{	// If top bit set, then it is a "run"
...
ILboolean iSaveSgiInternal()
{
	...
	if(iCurImage->Format != IL_LUMINANCE //while the sgi spec doesn't directly forbid rgb files with 2
	   //channels, they are quite uncommon and most apps don't support
	   //them. so convert lum_a images to rgba before writing.
	   //&& iCurImage->Format != IL_LUMINANCE_ALPHA
	   &&
	   iCurImage->Format != IL_RGB && iCurImage->Format != IL_RGBA)
	{
