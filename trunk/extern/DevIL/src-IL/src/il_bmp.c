...
ILboolean iCheckBmp(BMPHEAD* Header)
{
	...
	if(Header->biPlanes > 1)						// Not sure...
		return IL_FALSE;
	if(Header->biCompression != 0 && Header->biCompression != 1 && Header->biCompression != 2 && Header->biCompression != 3)
		return IL_FALSE;
...
ILboolean iCheckOS2(OS2_HEAD* Header)
{
	...
	if(Header->cx == 0 || Header->cy == 0)
		return IL_FALSE;
	if(Header->cBitCount != 1 && Header->cBitCount != 4 && Header->cBitCount != 8 && Header->cBitCount != 24)
		return IL_FALSE;
...
ILboolean ilReadUncompBmp(BMPHEAD* Header)
{
	...
	case 8:
		...
		{	// biClrUsed is 0 for 4-bit bitmaps
	...
	case 1:
		...
		for(j = 0; j < iCurImage->Height; j++)
		{
			...
			for(i = 0; i < iCurImage->Width;)
			{
				...
				for(c = 0; c < 8; c++)
				{
					iCurImage->Data[j * iCurImage->Width + i] = (!!(ByteData & k) == 1 ? 1 : 0);
					k >>= 1;
	...
	case 24:
		...
		{	// M$ requires lines to be padded if the widths aren't multiples of 4.
			//changed 2003-09-01
			if(iGetHint(IL_MEM_SPEED_HINT) == IL_FASTEST)
			{
				iPreCache(iCurImage->Width * iCurImage->Height);
			...
			{	// Escape sequence
				...
				{						// Run of pixels
					if((ILint) iCurImage->Width - (ILint) x < (ILint) Bytes[1])
						return IL_FALSE;
					if(iread(iCurImage->Data + (y * iCurImage->Width + x), IL_MIN(((ILint) iCurImage->Width - (ILint) x), Bytes[1]), 1) != 1)
						return IL_FALSE;
					x += Bytes[1];
					if(Bytes[1] % 2)
...
ILboolean iSaveBitmapInternal()
{
	...
	if(compress_rle8 == IL_TRUE)
	{
		...
		FileSize = ilRleCompress(TempImage->Data, TempImage->Width, TempImage->Height, TempImage->Depth, TempImage->Bpp, Dest, IL_BMPCOMP, NULL);
	...
	else
	{
		...
		{				// Odd width, so we must pad each line.
