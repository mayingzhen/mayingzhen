...
#warning DevIL was designed with libjpeg 6 b or higher in mind.Consider upgrading at www.ijg.org
#endif
#else
#include <ijl.h>
#include <limits.h>
...
ILboolean ilIsValidJpg(const ILstring FileName)
{
	...
	if(!iCheckExtension(FileName, "jpg") && !iCheckExtension(FileName, "jpe") && !iCheckExtension(FileName, "jpeg"))
	{
...
#ifndef IL_USE_IJL \
	// Use libjpeg instead of the IJL.
...
devil_jpeg_read_init(j_decompress_ptr cinfo)
{
	...
	{	// first time for this JPEG object?
		cinfo->src = (struct jpeg_source_mgr*)(*cinfo->mem->alloc_small) ((j_common_ptr) cinfo, JPOOL_PERMANENT, sizeof(iread_mgr));
...
ILboolean iSaveJpegInternal(const ILstring FileName, ILvoid* Lump, ILuint Size)
{
	...
	if((iCurImage->Format != IL_RGB && iCurImage->Format != IL_RGBA && iCurImage->Format != IL_LUMINANCE) || iCurImage->Bpc != 1)
	{
