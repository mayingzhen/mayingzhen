...
ILboolean ilFlipImage()
{
	...
	iCurImage->Origin = (iCurImage->Origin == IL_ORIGIN_LOWER_LEFT) ? IL_ORIGIN_UPPER_LEFT : IL_ORIGIN_LOWER_LEFT;
...
ILboolean ilCopyPixels2D(ILuint XOff, ILuint YOff, ILuint Width, ILuint Height, ILvoid* Data)
{
	...
	for(y = 0; y < NewHeight; y++)
	{
		for(x = 0; x < NewBps; x += PixBpp)
		{
			for(c = 0; c < PixBpp; c++)
			{
				Temp[y * DataBps + x + c] = TempData[(y + YOff) * iCurImage->Bps + x + NewXOff + c];
			}
		}
	}
	...
	for(z = 0; z < NewD; z++)
	{
		for(y = 0; y < NewH; y++)
		{
			for(x = 0; x < NewBps; x += PixBpp)
			{
				for(c = 0; c < PixBpp; c++)
				{
					Temp[z * NewSizePlane + y * DataBps + x + c] = TempData[(z + ZOff) * iCurImage->SizeOfPlane + (y + YOff) * iCurImage->Bps + x + NewXOff + c];
...
ILboolean ilSetPixels2D(ILint XOff, ILint YOff, ILuint Width, ILuint Height, ILvoid* Data)
{
	...
	for(y = 0; y < NewHeight; y++)
	{
		for(x = 0; x < NewWidth; x++)
		{
			for(c = 0; c < PixBpp; c++)
			{
				TempData[(y + YOff) * iCurImage->Bps + (x + XOff) * PixBpp + c] = Temp[(y + SkipY) * NewBps + (x + SkipX) * PixBpp + c];
			}
		}
	}
	...
	for(z = 0; z < NewD; z++)
	{
		for(y = 0; y < NewH; y++)
		{
			for(x = 0; x < NewW; x++)
			{
				for(c = 0; c < PixBpp; c++)
				{
					TempData[(z + ZOff) * iCurImage->SizeOfPlane + (y + YOff) * iCurImage->Bps + (x + XOff) * PixBpp + c] = Temp[(z + SkipZ) * NewSizePlane + (y + SkipY) * NewBps + (x + SkipX) * PixBpp + c];
				}
			}
		}
	}
...
ILvoid ILAPIENTRY ilSetPixelsAlphaTest(ILint XOff, ILint YOff, ILuint Width, ILuint Height, ILenum Format, ILenum Type, ILvoid* Data)
{
	ILvoid*	 Converted;
	ILuint c, SkipX = 0, SkipY = 0, NewBps, PixBpp;
	ILint x, y, NewWidth, NewHeight;
	ILubyte*  Temp, *TempData;
	...
	{
		Converted = ilConvertBuffer(Width * Height * ilGetBppFormat(Format) * ilGetBpcType(Type), Format, iCurImage->Format, iCurImage->Type, Type, Data);
		if(!Converted)
			return;
	}

	// Begin ilSetPixel2D
	Temp = (ILubyte*)Converted;
	TempData = iCurImage->Data;
	PixBpp = iCurImage->Bpp * iCurImage->Bpc;
	...
	for(y = 0; y < NewHeight; y++)
	{
		for(x = 0; x < NewWidth; x++)
		{
			unsigned char ucTestAlpha = Temp[(y + SkipY) * NewBps + (x + SkipX) * PixBpp + 3];
			if(ucTestAlpha != 0)
			{
				for(c = 0; c < PixBpp; c++)
				{
...
ILvoid ILAPIENTRY ilSetPixelsAlphaBlend(ILint XOff, ILint YOff, ILuint Width, ILuint Height, ILenum Format, ILenum Type, ILvoid* Data)
{
	ILvoid*	 Converted;
	ILuint c, SkipX = 0, SkipY = 0, NewBps, PixBpp;
	ILint x, y, NewWidth, NewHeight;
	ILubyte*  Temp, *TempData, *pv;
	...
	for(y = 0; y < NewHeight; y++)
	{
		for(x = 0; x < NewWidth; x++)
		{
			...
			for(c = 0; c < 3; c++)
			{
				pv = TempData + (y + YOff) * iCurImage->Bps + (x + XOff) * PixBpp + c;
				*pv = *pv * (255 - ucTestAlpha) / 255 + Temp[(y + SkipY) * NewBps + (x + SkipX) * PixBpp + c] * ucTestAlpha / 255;
			}

			pv = TempData + (y + YOff) * iCurImage->Bps + (x + XOff) * PixBpp + 3;
			if(*pv + ucTestAlpha > 255)
			{
				*pv = 255;
			}
			else
