    ...
    _timePerEmission(PARTICLE_EMISSION_RATE_TIME_INTERVAL), _timeRunning(0)
{
    ...
    _spriteBatch->getStateBlock()->depthWrite = false;//->setDepthWrite(false);
    _spriteBatch->getStateBlock()->depthTest = true; // setDepthTest(true);
}
...
void ParticleEmitter::setTextureBlending(TextureBlending textureBlending)
{
    ...
    {
        case BLEND_OPAQUE:
            _spriteBatch->getStateBlock()->_blendEnabled = false;
            break;
        case BLEND_TRANSPARENT:
            _spriteBatch->getStateBlock()->_blendEnabled = true;
            _spriteBatch->getStateBlock()->_blendSrc = BLEND_SRC_ALPHA; //setBlendSrc(RenderState::BLEND_SRC_ALPHA);
            _spriteBatch->getStateBlock()->_blendDst = BLEND_ONE_MINUS_SRC_ALPHA;// setBlendDst(RenderState::BLEND_ONE_MINUS_SRC_ALPHA);
            break;
        case BLEND_ADDITIVE:
            _spriteBatch->getStateBlock()->_blendEnabled = true;  //setBlend(true);
            _spriteBatch->getStateBlock()->_blendSrc = BLEND_SRC_ALPHA; //setBlendSrc(RenderState::BLEND_SRC_ALPHA);
            _spriteBatch->getStateBlock()->_blendDst = BLEND_ONE;// setBlendDst(RenderState::BLEND_ONE);
            break;
        case BLEND_MULTIPLIED:
            _spriteBatch->getStateBlock()->_blendEnabled = true; //setBlend(true);
            _spriteBatch->getStateBlock()->_blendSrc = BLEND_ZERO; //setBlendSrc(RenderState::BLEND_ZERO);
            _spriteBatch->getStateBlock()->_blendDst = BLEND_SRC_COLOR; //(RenderState::BLEND_SRC_COLOR);
            break;
        default:
...
void ParticleEmitter::draw(Camera* pCamera)
{
    ...
    if (_particleCount > 0)
    {
        ...
        NodeTransform tsfWS = pCamera->GetTransform();
		Matrix4x4 cameraWorldMatrix ;//_node->getScene()->getActiveCamera()->getNode()->getWorldMatrix();
		MatrixFromTransform(&cameraWorldMatrix,&tsfWS);
