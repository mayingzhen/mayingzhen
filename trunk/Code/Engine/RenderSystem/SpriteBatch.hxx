...
static Technique* __spriteEffect = NULL;
...
SpriteBatch* SpriteBatch::create(const char* texturePath, Technique* effect, unsigned int initialCapacity)
{
...
SpriteBatch* SpriteBatch::create(Texture* texture,  Technique* effect, unsigned int initialCapacity)
{
    ASSERT(texture != NULL);
    ...
    if (!customEffect)
    {
        ...
        if (__spriteEffect == NULL)
        {
			...
			__spriteEffect = GetRenderDevice()->CreateTechnique();//new Technique();
			__spriteEffect->Load(NULL,SPRITE);
            if (__spriteEffect == NULL)
            {
...
//     }
    ...
    material->GetRenderState()._blendEnabled = true; //->setBlend(true);
    material->GetRenderState()._blendSrc = BLEND_SRC_ALPHA; // ->setBlendSrc(RenderState::BLEND_SRC_ALPHA);
    material->GetRenderState()._blendDst = BLEND_ONE_MINUS_SRC_ALPHA; // ->setBlendDst(RenderState::BLEND_ONE_MINUS_SRC_ALPHA);
    ...
    batch->_sampler = texture;//sampler;
...
{
    return NULL;//& _batch->getMaterial()->GetRenderState();
}
...
{
    return  NULL;//_batch->getMaterial();
}
