...
void CmdLineOptionCollection::SetOption(eCmdLineOptionType option)
{
	if (option < CLO_NUM )
	{
		mCurrent[option].present = true;
	}
}
										   ...
										   const std::vector<std::string> vValue)
{
	...
	{
		mCurrent[option].present = true;
		mCurrent[option].pStartArgs = vValue;
	}
}
...
//-----------------------------------------------------------------------------
const char *  CmdLineOptionCollection::GetArgument(eCmdLineOptionType option, int n) const
{
    if ((option < CLO_NUM) && (n < kNumArguments[option]))
        return (mCurrent[option].pStartArgs[n]).c_str();
    return NULL;
}
...
{
    assert( i >= 0 && i < mpFilenames.size() );
    *ppFilename = mpFilenames[i].c_str();
}
...
bool CmdLineOptionCollection::IsArgPowerOf2(eCmdLineOptionType option) const
{
    ...
    int         resolution;    
    int const   kError = sscanf(mCurrent[option].pStartArgs[0].c_str(), "%i", &resolution);
