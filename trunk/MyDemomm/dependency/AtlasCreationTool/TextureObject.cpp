...
//       ws mapped to.
//-----------------------------------------------------------------------------
void Texture2D::WriteTAILine(CmdLineOptionCollection const &options, FILE *fp) 
{
	...
	mInAlta.mAltaID = id;
	mInAlta.mAltaType = mpAtlas->GetType();
	mInAlta.mpFilename = mpFilename;
	mInAlta.mpAtlasName = (mpAtlas == NULL) ? mpFilename : mpAtlas->GetFilename();
	mInAlta.uOffset = uOffset;
	mInAlta.vOffset = vOffset;
	mInAlta.wOffset = wOffset;
	mInAlta.uWidth = uWidth;
	mInAlta.vHeight = vHeight;
}
...
void AtlasVolume::Shrink()
{
    ...
    int const   kPowerOf2Depth = (kRemainder == 0.0f) ? mpPackerVolume->GetSlicesUsed() 
                                                      : static_cast<const int>(pow(2.0f, static_cast<int>(kLog2)+1));
