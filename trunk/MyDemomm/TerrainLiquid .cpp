...
void TerrainLiquid::create(const D3DXVECTOR3& seedPos, 
						   float fTextureScale, bool bDepthEnable, 
						   float fDepthScale, CTerrain* pTerrainSystem )
{
	// init parameter
	mSeedPos = seedPos;
	mStep = D3DXVECTOR2(pTerrainSystem->m_scale.x, pTerrainSystem->m_scale.z);
...
// 	m_worldRect.z1 = MIN_REAL32;
	...
	for( it = m_pTempGridArray.begin(); it != m_pTempGridArray.end(); it++, i += 4 )
	{
		...
		pVerts[i].vNormal = D3DXVECTOR3( 0.0f, 1.0f, 0.0f );
		...
		pVerts[i + 1].vNormal = D3DXVECTOR3( 0.0f, 1.0f, 0.0f );
		...
		pVerts[i + 2].vNormal = D3DXVECTOR3( 0.0f, 1.0f, 0.0f );
		...
		pVerts[i + 3].vNormal = D3DXVECTOR3( 0.0f, 1.0f, 0.0f );
...
bool TerrainLiquid::isValidGrid( int x, int z, int dir )
{
	...
	D3DXVECTOR3 leftTop = getPos( x, z );
	D3DXVECTOR3 rightTop = getPos( x + 1, z );
	D3DXVECTOR3 leftBottom = getPos( x, z + 1);
	D3DXVECTOR3 rightBottom = getPos( x + 1, z + 1 );
...
D3DXVECTOR2 TerrainLiquid::getTexCoord( int x, int z, int type )
{
	return D3DXVECTOR2( x * m_fTextureScale, z * m_fTextureScale );
}

float TerrainLiquid::getDepthTexCoord( int x, int z )
{
	D3DXVECTOR3 pos = getPos(x,z);
