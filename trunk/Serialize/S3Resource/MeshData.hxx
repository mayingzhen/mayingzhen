...
{
	for (UINT nCnt = 0; nCnt < (UINT)arr.size(); ++nCnt)
	{
...
void PointerArrayResize(std::vector<ElementType*>& arr,UINT nNewSize)
{
	UINT nOldSize = (UINT)arr.size();
	for (UINT nCnt = nNewSize; nCnt < nOldSize; ++nCnt)
	{
		SAFE_DELETE(arr[nCnt]);
	}
	arr.resize(nNewSize);
	for (UINT nCnt = nOldSize; nCnt < nNewSize; ++nCnt)
	{
			  ...
			  ,const char* pszTag )
{
	...
	if (arrBonePalatte)
	{
		for (UINT nPalCnt = 0; nPalCnt < nBonePalatteSize; ++nPalCnt)
		{
...
void	SubMeshData::ResetBonePalatte(xmUint32 nPalatteSize,xmUint16* arrBoneIndice)
{
	...
	{
		for (UINT nPalCnt = 0; nPalCnt < m_arrBonePalette.size(); ++nPalCnt)
		{
...
const SubMeshData*	MeshLODData::GetSubMesh(UINT nSubMeshInd) const
{
	return m_arrSubMesh[nSubMeshInd];
}


void					MeshLODData::ResizeSubMesh(UINT nSubMeshNum)
{
...
UINT					MeshLODData::GetSubMeshNumber() const
{
	return m_arrSubMesh.size();
}

SubMeshData*			MeshLODData::GetSubMesh(UINT nSubMeshInd)
{
...
UINT					MeshLODData::GetMeshDataFlag() const
{
	return m_nMeshDataFlag;
}

void					MeshLODData::SetMeshDataFlag(UINT uMeshDataFlag)
{
	m_nMeshDataFlag = uMeshDataFlag;
}

UINT					MeshLODData::GetIndexNumber() const
{
...
void					MeshLODData::ResetBuffer(bool bIsIndex32, xmUint32 nIndexNum, xmUint32 nVertexNum)
{
	...
	const UINT nIndexStride = m_nIndexType == INDEX_TYPE_U16 ? sizeof(xmUint16) : sizeof(xmUint32);
	const UINT nVertexStride = sizeof(VertexType0);
...
{
	for (UINT nSubMeshCnt = 0; nSubMeshCnt < (UINT)m_arrSubMesh.size(); ++nSubMeshCnt)
	{
...
UINT				MeshData::GetVertexStride() const
{
	return sizeof(VertexType0);
}


UINT				MeshData::GetBoneIndexOffset() const
{
	VertexType0* pData = NULL;
	UINT nOffset = (xmUint8*)(&pData->b) - (xmUint8*)(pData);
	return nOffset;
}

UINT				MeshData::GetBoneWeightOffset() const
{
	VertexType0* pData = NULL;
	UINT nOffset = (xmUint8*)(&pData->w) - (xmUint8*)(pData);
	return nOffset;
}
...
void MeshData::DegenerateSubMeshData( MeshLODData * pLODData, std::vector<unsigned int> &subIndexMove ) 
{
	for (int i = 0; i < (int)pLODData->GetSubMeshNumber(); ++i)
	{
		...
		{
			UINT indexStart = pSubMeshData->GetIndexStart();
...
void MeshData::RemoveDegenerateTriangleInplace(std::vector<xmUint8> &arrIndexBuffer, UINT nIndexType, std::vector<unsigned int> &subIndexMove)
{
...
// UINT				MeshData::GetBoneNumber() const
// {
// 	return m_arrBoneName.size();
// }

// const char*			MeshData::GetBoneName(UINT nBoneInd) const
// {
...
//For LOD Mesh
//------------------------------------------------------------------------------
UINT                MeshData::GetLODMeshNumber() const
{
...
//
//------------------------------------------------------------------------------
UINT				MeshData::GetSubMeshNumber() const
{
	return GetLODMesh(0)->GetSubMeshNumber();
}


const SubMeshData*	MeshData::GetSubMesh(UINT nSubMeshInd, int nLOD) const
{
...
void					MeshData::ResetBuffer(UINT nIndexType,xmUint32 nIndexNum
									,xmUint32 nVertexNum
									...
									)
{
	...
	const UINT nIndexStride = nIndexType == INDEX_TYPE_U16 ? sizeof(xmUint16) : sizeof(xmUint32);
...
SubMeshData*		MeshData::GetSubMesh(UINT nSubMeshInd, int nLOD)
{
...
SubMeshData*		MeshData::GetSubMeshByName(const char* pszName, int nLOD)
{
	...
	const UINT nSubMeshNum = GetSubMeshNumber();
