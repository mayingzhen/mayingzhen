...
// 	GL_ASSERT( glBindAttribLocation(program,DU_TEXCOORD,VERTEX_ATTRIBUTE_TEXCOORD_PREFIX_NAME) );
...
void Effect::setValue(Uniform* uniform, const Matrix4x4& value)
{
    GP_ASSERT(uniform);
    GL_ASSERT( glUniformMatrix4fv(uniform->_location, 1, GL_FALSE, (GLfloat*)&value) );
}

void Effect::setValue(Uniform* uniform, const Matrix4x4* values, unsigned int count)
{
    GP_ASSERT(uniform);
    GP_ASSERT(values);
...
void Effect::setValue(Uniform* uniform, const Texture* smpler/*const Texture::Sampler* sampler*/)
{
    GP_ASSERT(uniform);
	...
	GLESTexture* pTexture = (GLESTexture*)smpler;
	pTexture->bind();
