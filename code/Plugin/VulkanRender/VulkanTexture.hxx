...
// 			0 );
	...
	bool VulkanTexture::LoadFromImagData(const ImageData& imageData)
	{
		...
		m_eFormat = VulkanMapping::_getClosestSupportedPF(imageData.format);
		PixelBox src = imageData.GetPixelBox(0, 0);
		RefPtr<MemoryStream> buf = NULL;
		if (m_eFormat != imageData.format)
		{
			buf = CreateMemoryStream(
				PixelUtil::getMemorySize(src.getWidth(), src.getHeight(), src.getDepth(), m_eFormat), false);

			PixelBox dst = PixelBox(src.getWidth(), src.getHeight(), src.getDepth(), m_eFormat, buf->GetPtr());
		...
		bufferCreateInfo.size = PixelUtil::getMemorySize(m_nWidth, m_nHeight, 1, m_eFormat);//imageData.size;
		...
		memcpy(data, src.data, PixelUtil::getMemorySize(m_nWidth, m_nHeight, 1, m_eFormat));
