#ifndef __IntersectionTests_H__
#define __IntersectionTests_H__ 
...
namespace ma
{
	...
	//
	//
	inline bool SweepIntersectionTest(const AABB &objectBB, const AABB &frustumBB, const Vector3 &vSweepDir)
	{
	  ...
	  float fFrustumCenterProj = Vec3Dot(vFrustumCenter, vSweepDir);
	  ...
	  float fObjectCenterProj = Vec3Dot(vObjectCenter, vSweepDir);
	  // project AABB half-size
	  ...
	  if(fDistMin > fDistMax) std::swap(fDistMin, fDistMax);
	  ...
	  else
	  {
		...
		if(fDistMinNew > fDistMaxNew) std::swap(fDistMinNew, fDistMaxNew);
									  ...
									  const Vector3 &vDir)
	{
	  ...
	  float fCenter = Vec3Dot(vCenter, vDir);
	  ...
	  float fProj2 = Vec3Dot(pFrustumPoints[0], vDir);
	  float fMin2 = fProj2;
	  float fMax2 = fProj2;
	  for(int i=1;i<8;i++)
	  {
		fProj2 = Vec3Dot(pFrustumPoints[i], vDir);
	...
	//
	//
	inline bool IntersectionTest(const AABB &objectBB, const Frustum &frustum)
	{
	  ...
	  {
		// edge up-down
		Vector3 vNorm1 = Vec3Normalize(Vec3Cross(frustum.m_pPoints[1] - frustum.m_pPoints[0], pBoxEdges[i]));
		if(!ProjectedIntersection(vHalfSize, vCenter, frustum.m_pPoints, vNorm1)) return false;

		// edge left-right
		Vector3 vNorm2 = Vec3Normalize(Vec3Cross(frustum.m_pPoints[3] - frustum.m_pPoints[0], pBoxEdges[i]));
		if(!ProjectedIntersection(vHalfSize, vCenter, frustum.m_pPoints, vNorm2)) return false;

		// edge bottom left
		Vector3 vNorm3 = Vec3Normalize(Vec3Cross(frustum.m_pPoints[4] - frustum.m_pPoints[0], pBoxEdges[i]));
		if(!ProjectedIntersection(vHalfSize, vCenter, frustum.m_pPoints, vNorm3)) return false;

		// edge top left
		Vector3 vNorm4 = Vec3Normalize(Vec3Cross(frustum.m_pPoints[5] - frustum.m_pPoints[1], pBoxEdges[i]));
		if(!ProjectedIntersection(vHalfSize, vCenter, frustum.m_pPoints, vNorm4)) return false;

		// edge top right
		Vector3 vNorm5 = Vec3Normalize(Vec3Cross(frustum.m_pPoints[6] - frustum.m_pPoints[2], pBoxEdges[i]));
		if(!ProjectedIntersection(vHalfSize, vCenter, frustum.m_pPoints, vNorm5)) return false;

		// edge bottom right
		Vector3 vNorm6 = Vec3Normalize(Vec3Cross(frustum.m_pPoints[7] - frustum.m_pPoints[3], pBoxEdges[i]));
									  ...
									  float &fDistMin, float &fDistMax, const Vector3 &vSweepDir)
	{
	  ...
	  float fSweepDir = Vec3Dot(vSweepDir, vDir);
	  ...
	  if(fSweepDir != 0)
	  {
		...
		if(fIntersectionStart > fIntersectionEnd) std::swap(fIntersectionStart, fIntersectionEnd);
	...
	//
	//
	inline bool SweepIntersectionTest(const AABB &objectBB, const Frustum &frustum, const Vector3 &vSweepDir)
	{
	  // Note that this code is very unoptimal
	  //
	  //
