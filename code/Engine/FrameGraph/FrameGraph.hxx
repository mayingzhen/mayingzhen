...
const char* FrameGraph::Builder::getName(FrameGraphHandle const& r) const noexcept 
{
    ...
    ResourceEntryBase* pResource = resourceNode.resource;
    assert(pResource);
...
FrameGraphHandle FrameGraph::createResourceNode(ResourceEntryBase* resource) noexcept
{
    auto& resourceNodes = mResourceNodes;
    auto& resourceNodeEntries = mResourceNodeEntries;
    size_t index = resourceNodes.size();
    ResourceNode* pBase = new ResourceNode(resource, resource->version);
    resourceNodeEntries.emplace_back(pBase);
...
FrameGraphHandle FrameGraph::create(ResourceEntryBase* pResourceEntry) noexcept 
{
    mResourceEntries.emplace_back(pResourceEntry);
...
{
    ASSERT(r.isValid() && "using an uninitialized resource handle");
    ResourceNode& node = getResourceNodeUnchecked(r);
    ASSERTMSG(node.resource->version == node.version,
...
ResourceEntryBase& FrameGraph::getResourceEntryBase(FrameGraphHandle r) noexcept 
{
...
ResourceEntryBase& FrameGraph::getResourceEntryBaseUnchecked(FrameGraphHandle r) noexcept {
    ResourceNode& node = getResourceNodeUnchecked(r);
    assert(node.resource);
    return *node.resource;
}
...
{
    Vector<PassNode>& passNodes = mPassNodes;
    Vector<ResourceNode*>& resourceNodes = mResourceNodes;
    Vector<UniquePtr<ResourceEntryBase>>& resourceRegistry = mResourceEntries;
    ...
    for (PassNode& pass : passNodes) 
	{
        if (pass.refCount) 
		{
            for (auto handle : pass.samples) 
			{
                ...
                //texture.descriptor.usage |= TEXTURE_USAGE::USAGE_SAMPLEABLE;
            }
        }
    }

    for (UniquePtr<ResourceEntryBase> const& resource : resourceRegistry)
	{
        if (resource->refs) 
		{
...
void FrameGraph::executeInternal(PassNode const& node) noexcept 
{
    assert(node.base);
...
void FrameGraph::export_graphviz(std::ostream& out, const char* viewName) 
{
#ifndef NDEBUG
