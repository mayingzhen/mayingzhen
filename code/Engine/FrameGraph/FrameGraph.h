...
public:
    ...
    public:
        ...
        template<>
        FrameGraphId<FrameGraphRenderTarget> create(const char* name, typename FrameGraphRenderTarget::Descriptor const& desc) noexcept 
		{
            return createRenderTargetImpl(name, desc);
        }

        // Helper to create a Texture resource
        FrameGraphId<FrameGraphTexture> createTexture(const char* name, FrameGraphTexture::Descriptor const& desc = {}) noexcept 
		{
            return create<FrameGraphTexture>(name, desc);
        }

        // Helper to create a RenderTarget resource
        FrameGraphId<FrameGraphRenderTarget> createRenderTarget(const char* name, FrameGraphRenderTarget::Descriptor const& desc) noexcept 
		{
        ...
        template<typename T,typename = std::enable_if_t<!std::is_same<T, FrameGraphRenderTarget>::value>>
    ...
    private:
        ...
        FrameGraphId<FrameGraphRenderTarget> createRenderTargetImpl(const char* name, typename FrameGraphRenderTarget::Descriptor const& desc) noexcept;
    ...
    typename T::Descriptor& getDescriptor(FrameGraphId<T> r) 
	{
        ResourceEntry<T>& entry = getResourceEntryUnchecked(r);
        return entry.descriptor;
    }

    template<typename T>
    FrameGraphId<T> import(const char* name,typename T::Descriptor const& desc, const T& resource) noexcept 
	{
        ResourceEntryBase* pBase = new ResourceEntry<T>(name, desc, resource, mId++, 0);
    ...
    FrameGraphId<FrameGraphRenderTarget> import(const char* name,
        ...
        ResourceEntryBase* pBase = new RenderTargetResourceEntry(
...
//     };
   ...
   // LinearAllocatorArena& getArena() noexcept { return mArena; }
    ...
    PassNode& createPass(const char* name, FrameGraphPassExecutor* base) noexcept;

    FrameGraphHandle createResourceNode(ResourceEntryBase* resource) noexcept;

    void executeInternal(PassNode const& node) noexcept;

    ResourceAllocator& getResourceAllocator() noexcept { return mResourceAllocator; }

    void reset() noexcept;

    void moveResourceBase(FrameGraphHandle from, FrameGraphHandle to);

    FrameGraphHandle create(ResourceEntryBase* pResourceEntry) noexcept;

    template<typename T>
    FrameGraphId<T> create(const char* name, typename T::Descriptor const& desc) noexcept 
	{
        ResourceEntryBase* pBase = new ResourceEntry<T>(name, desc, mId++, 0);
	...
	{
		ResourceEntryBase* pBase = new RenderTargetResourceEntry(name, desc, mId++, 1);
        FrameGraphId<FrameGraphRenderTarget> r(create(pBase));
        return r;
    }

    ResourceNode& getResourceNode(FrameGraphHandle r);
    ResourceNode& getResourceNodeUnchecked(FrameGraphHandle r);

    ResourceEntryBase& getResourceEntryBase(FrameGraphHandle r) noexcept;
    ResourceEntryBase& getResourceEntryBaseUnchecked(FrameGraphHandle r) noexcept;

    template<typename T>
    ResourceEntry<T>& getResourceEntry(FrameGraphId<T> r) noexcept {
        return static_cast<ResourceEntry<T>&>(getResourceEntryBase(r));
    }

    template<typename T>
    ResourceEntry<T>& getResourceEntryUnchecked(FrameGraphId<T> r) noexcept {
        return static_cast<ResourceEntry<T>&>(getResourceEntryBaseUnchecked(r));
    }

    Blackboard mBlackboard;
    ResourceAllocator& mResourceAllocator;

    Vector<PassNode> mPassNodes;                    // list of frame graph passes
    Vector<ResourceNode *> mResourceNodes;          // list of resource nodes
    Vector<UniquePtr<ResourceNode>> mResourceNodeEntries;
    Vector<UniquePtr<ResourceEntryBase>> mResourceEntries;
    uint16_t mId = 0;
};
