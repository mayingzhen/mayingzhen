...
bool Container::moveFocus(Direction direction, Control* outsideControl)
{
    ...
    if (start)
    {
        ...
        {
        case UP:
            vStart = Vector2(startBounds.x + startBounds.width * 0.5f,
                        startBounds.y);
            break;
        case DOWN:
            vStart = Vector2(startBounds.x + startBounds.width * 0.5f,
            ...
            break;
        case LEFT:
            vStart = Vector2(startBounds.x,
                        startBounds.y + startBounds.height * 0.5f);
            break;
        case RIGHT:
            vStart = Vector2(startBounds.right(),
                        startBounds.y + startBounds.height * 0.5f);
            break;
        case NEXT:
            break;
        ...
        if (direction != NEXT)
        {
            ...
            for (itt = _controls.begin(); itt < _controls.end(); itt++)
            {
                ...
                {
                case UP:
                    vNext = Vector2(nextBounds.x + nextBounds.width * 0.5f,
                              nextBounds.bottom());
...
{
    double time = 0;//Game::getAbsoluteTime();
...
{
    _scrollingVelocity = Vector2(-x, y);
    _scrolling = true;
...
{
    _scrollingVelocity = Vector2(0, 0);
    _scrolling = false;
    _dirty = true;
}
...
void Container::updateScroll()
{
    ...
    double frameTime = 0;//Game::getGameTime();
...
//             }
    ...
    case Touch::TOUCH_MOVE:
        ...
        {
            double gameTime = 0;//Game::getAbsoluteTime();
            ...
            if (_scrollingMouseVertically)
            {
                ...
                _scrollingVelocity = Vector2(0, -vy);
            ...
            else if (_scrollingMouseHorizontally)
            {
                ...
                _scrollingVelocity = Vector2(-vx, 0);
                _scrollPosition.x -= vx;
            }
            else
            {
                _scrollingVelocity = Vector2(vx, vy);
                _scrollPosition.x += vx;
                _scrollPosition.y += vy;
            }
