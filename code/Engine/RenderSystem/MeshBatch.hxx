...
void MeshBatch::add(const void* vertices, size_t size, unsigned int vertexCount, const unsigned short* indices, unsigned int indexCount)
{
    ...
    memcpy(m_pVbLock, vertices, vBytes);
    ...
    if (_indexed)
    {
        ...
        if (_vertexCount == 0)
        {
            ...
            memcpy(m_pIbLock, indices, indexCount * sizeof(unsigned short));
        }
        else
        {
            if (m_ePrimitiveType == PRIM_TRIANGLESTRIP)
            {
                ...
                m_pIbLock[0] = *(m_pIbLock-1);
                m_pIbLock[1] = _vertexCount;
                m_pIbLock += 2;
            }
            ...
            {
                m_pIbLock[i] = indices[i] + _vertexCount;
            }
        }

		m_pIbLock += indexCount;
...
bool MeshBatch::resize(unsigned int capacity)
{
	...
	//m_pIbLock = (unsigned short*)m_pIndexBuffer->Lock(0,m_pVertexBuffers->GetSize(),LOCK_DISCARD);
	//m_pVbLock = m_pVertexBuffers->Lock(0,m_pVertexBuffers->GetSize(),LOCK_DISCARD);
...
void MeshBatch::start()
{
	...
	m_pIbLock = (unsigned short*)m_pIndexBuffer->Lock(0,m_pIndexBuffer->GetSize(),LOCK_DISCARD);
	m_pVbLock = (unsigned char*)m_pVertexBuffers->Lock(0,m_pVertexBuffers->GetSize(),LOCK_DISCARD);
}
...
// 	Technique* pTech = m_pMaterial->GetTechnique();
// 	pTech->Begin();
// 
// 	pTech->CommitChanges();
	//pTech->SetTexture("u_texture",m_pMaterial->GetDiffuseMap());
	//pTech->SetMatrix("u_projectionMatrix",&mProjMatrix);
		// 	
