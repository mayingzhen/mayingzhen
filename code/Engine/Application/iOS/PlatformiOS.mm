...
- (void)touchesBegan:(NSSet*)touches withEvent:(UIEvent*)event 
{
    ...
    for(UITouch* touch in touches) 
    {
        ...
        if (i < TOUCH_POINTS_MAX)
        {
            ...
            game->touchEvent(Touch::TOUCH_PRESS, __touchPoints[i].x, __touchPoints[i].y, i);
        }
        else
        {
    ...
    for(UITouch* touch in touches) 
    {
        ...
        for (int i = 0; !found && i < TOUCH_POINTS_MAX; i++)
        {
            if (__touchPoints[i].down && __touchPoints[i].hashId == touchID)
            {
                ...
                game->touchEvent(Touch::TOUCH_RELEASE, touchPoint.x * WINDOW_SCALE, touchPoint.y * WINDOW_SCALE, i);
                found = true;
            }
        }
        ...
        if (!found)
        {
            ...
            for (int i = 0; i < TOUCH_POINTS_MAX; i++)
            {
                ...
                {
                    __touchPoints[i].down = false;
                    game->touchEvent(Touch::TOUCH_RELEASE, __touchPoints[i].x, __touchPoints[i].y, i);
                }
            }
        }
    }
    ...
    for(UITouch* touch in touches) 
    {
        ...
        for (int i = 0; i < TOUCH_POINTS_MAX; i++)
        {
            if (__touchPoints[i].down && __touchPoints[i].hashId == touchID)
            {
                __touchPoints[i].x = touchPoint.x * WINDOW_SCALE;
                __touchPoints[i].y = touchPoint.y * WINDOW_SCALE;
                game->touchEvent(Touch::TOUCH_MOVE, __touchPoints[i].x, __touchPoints[i].y, i);
                break;
            }
        }
    }
