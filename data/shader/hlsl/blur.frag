...
float4 CalcBlur(float4 iTex0, float4 iTex1, float4 iTex2, float4 iTex3, float2 offset)
{
	...
	{
		tex[i] += offset.xyxy;
		color.rgb += g_SamplerSrc.Sample(s_SamplerSrc, tex[i].xy).rgb * color_weight[i * 2 + 0];
		color.rgb += g_SamplerSrc.Sample(s_SamplerSrc, tex[i].zw).rgb * color_weight[i * 2 + 1];
	}
...
float4 ps_main(float4 iTex0 : TEXCOORD0,
				...
				float2 iOriTex : TEXCOORD4) : SV_TARGET
{
